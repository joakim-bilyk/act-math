...
#Estimates
plotdf <- data.frame(
  times = rep(unlist(lapply(1:length(results), function(i) results[[i]]$times)),3),
  matrix(unlist(lapply(1:length(results), function(i) results[[i]]$Aalen_Johansen)),ncol=3,byrow=TRUE)
)
