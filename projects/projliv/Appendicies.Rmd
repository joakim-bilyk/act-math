...
s <- 5
j <- 1
approximation_risk <- function(paths,results,Ls,s,j,p=2,num_states,debug = FALSE, T_max,increments=100) {
  if (debug) {
    print(paste0(Sys.time()," Calculating Markov estimate."))
    start_time <- Sys.time()
  }
  ...
  plots <- lapply(1:num_states, function(i) {
    ...
    times_i <- data.frame(Time = p_con[,"Time"])
    ...
    for (k in 1:length(Ls)) {
        ...
        tmp <- merge(times_i,markov_tmp,all = TRUE,by = "Time")
        ...
        tmp3 <- data.frame(Time = (T_max-s)*0:increments/increments+s)
        tmp3[,"reference_time"] <- unlist(lapply(tmp3$Time, function(t) min(tmp$Time[tmp$Time>= t])))
        tmp3 <- tmp3 %>% filter(!is.infinite(reference_time))
        tmp3 <- merge(tmp3, tmp,all.x = TRUE,by.x = "reference_time",by.y = "Time")
        tmp3[,"Markov"] <- c(0,tmp3[2:dim(tmp3)[1],"Markov"] - tmp3[1:(dim(tmp3)[1]-1),"Markov"])
        tmp3[,"As_If"] <- c(0,tmp3[2:dim(tmp3)[1],"As_If"] - tmp3[1:(dim(tmp3)[1]-1),"As_If"])
        tmp3 <- tmp3[,c("Time","L","Markov","As_If")]
        
        tmp[,"L_order"] <- LETTERS[k]
        tmp2[,"L_order"] <- LETTERS[k]
        tmp3[,"L_order"] <- LETTERS[k]
        
        if (k > 1) {
          plotdf <- rbind(plotdf,tmp)
          plotdf2 <- rbind(plotdf2,tmp2)
          plotdf3 <- rbind(plotdf3,tmp3)
          norms <- rbind(norms, norms_tmp)
        } else {
          plotdf <- tmp
          plotdf2 <- tmp2
    ...
    labels <- data.frame(
      factor = c(paste0(unique(plotdf$L_order),", Markov"),paste0(unique(plotdf$L_order),", As-If")),
      label = c(paste0(unique(plotdf$L),", Markov"),paste0(unique(plotdf$L),", As-If"))
      ) %>% arrange(factor)
    
    plot1 <- ggplot(plotdf) +
      geom_line(mapping = aes(x=Time, y = Markov,col = factor(paste0(L_order,", Markov"))), linetype = "solid") +
      geom_line(mapping = aes(x=Time, y = As_If,col =  factor(paste0(L_order,", Af-If"))), linetype = "solid") +
      theme_bw() +
    ...
    plot3 <- ggplot(plotdf3) +
      geom_line(mapping = aes(x=Time, y = Markov,col = factor(paste0(L_order,", Markov"))), linetype = "solid") +
      geom_line(mapping = aes(x=Time, y = As_If,col =  factor(paste0(L_order,", Af-If"))), linetype = "solid") +
      theme_bw() +
      labs(title = TeX(paste0("Increments in accumulated norm for $p^{(L)}_{",letters[i],"}$")),
           y =  "Increments",
           x = "t",col = "L") +
      theme(plot.title = element_text(face = "bold"),
            plot.subtitle = element_text(face = "italic")) +
    ...
    plot4 <- ggplot(plotdf3) +
      geom_line(mapping = aes(x=Time, y = Markov-As_If,col = factor(L)), linetype = "solid") +
      theme_bw() +
      labs(title = TeX(paste0("Difference in increments in accumulated norm for $p^{(L)}_{",letters[i],"}$")),
           y = "Markov - As-If",
           x = "t",col = "L") +
      theme(plot.title = element_text(face = "bold"),
            plot.subtitle = element_text(face = "italic")) +
      scale_color_manual(values=c("#00008B","#7B68EE", "#6495ED","#87CEFA","#B0E0E6"))
  return(list(p1 = plot1, p2 = plot2, p3 = plot3, p4 = plot4,norms = norms))})
