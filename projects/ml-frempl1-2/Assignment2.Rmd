...
xgb_regr <- xgboost(data=as.matrix(df_sev %>% select(-ClaimAmount)), label=as.numeric(df_sev[,"ClaimAmount"]), eta = 0.246407, nrounds = 4000, max_depth = 3)
xgb_freq <- xgboost(data=as.matrix(df_freq %>% select(-ClaimInd)), label=as.numeric(df_freq[,"ClaimInd"]), eta = 0.196547, nrounds = 4000, max_depth = 3, objective="count:poisson")
```
...
```{r}
unified_xgb <- xgboost.unify(xgb_freq,data=as.matrix(df_freq %>% select(-ClaimInd)))

treeshap_xgb <- treeshap(unified_xgb,  df_freq %>% select(-ClaimInd), verbose = 0)

plot_contribution(treeshap_xgb, obs = )
plot_feature_importance(treeshap_xgb, max_vars = 10) 
...
exp(-4.79)/(exp(-4.79)+1)/0.083
```

Here we are getting a logit transformed frequency from the plot, so in order to reverse the function one has to do the following:
\begin{align*}
      logit(y)=x \Rightarrow y=\frac{\exp(x)}{exp(x)+1}
\end{align}