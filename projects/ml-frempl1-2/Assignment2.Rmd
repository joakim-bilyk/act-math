...
xgb_regr <- xgboost(data=as.matrix(df_sev), label=as.numeric(df_sev[,"ClaimAmount"]), ta = 0.246407, nrounds = 379, max_depth = 3)
xgb_freq <- xgboost(data=as.matrix(df_freq), label=as.numeric(df_freq[,"ClaimInd"]), ta = 0.196547, nrounds = 1747, max_depth = 3, objective="count:poisson")
```
...
treeshap_xgb <- treeshap(unified_xgb,  df_sev, verbose = 0)
