...
XGBoost_fitted_sev <- lrn("regr.xgboost", eta = 0.246407, nrounds = 379, max_depth = 3)
XGBoost_fitted_freq <- lrn("classif.xgboost", eta = 0.196547, nrounds = 1747, max_depth = 3, predict_type = "prob")

xgb_regr <- xgboost(data=as.matrix(df_sev[,-7]), label=as.numeric(df_sev[,1]), ta = 0.246407, nrounds = 379, max_depth = 3)
xgb_freq <- xgboost(data=as.matrix(df_freq[,-7]), label=as.numeric(df_freq[,1]), ta = 0.246407, nrounds = 379, max_depth = 3, objective="count:poisson")

XGBoost_fitted_sev$train(task_sev)
XGBoost_fitted_freq$train(task_freq)
```
...
```{r}
unified_xgb <- xgboost.unify(xgb_regr,data=as.matrix(df_freq[,-7]))

treeshap_xgb <- treeshap(unified_xgb,  df_sev[, -7], verbose = 0)
