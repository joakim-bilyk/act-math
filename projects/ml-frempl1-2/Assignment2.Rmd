...
c(6254,24123,25010,25188,30495,30555)
...
```{r}
xgb_regr <- xgboost(data=as.matrix(df_sev %>% dplyr::select(-ClaimAmount)), label=as.numeric(df_sev[,"ClaimAmount"]), eta = 0.246407, nrounds = 500, max_depth = 2)
xgb_freq <- xgboost(data=as.matrix(df_freq %>% dplyr::select(-ClaimInd)), label=as.numeric(df_freq[,"ClaimInd"]), eta = 0.196547, nrounds = 1700, max_depth = 2, objective="count:poisson")

xgb_gender_freq <- xgboost(data=as.matrix(df_gender_freq %>% dplyr::select(-ClaimInd)), label=as.numeric(df_gender_freq[,"ClaimInd"]), eta = 0.196547, nrounds = 1700, max_depth = 2, objective="count:poisson")
xgb_gender_sev <- xgboost(data=as.matrix(df_gender_sev %>% dplyr::select(-ClaimAmount)), label=as.numeric(df_gender_sev[,"ClaimAmount"]), eta = 0.246407, nrounds = 500, max_depth = 2)
```
...
unbias[c(6254,24123,25010,25188,30495,30555)]*skalar #unbiased
predict(xgb_regr, newdata = as.matrix(df[c(6254,24123,25010,25188,30495,30555),]%>% dplyr::select(-ClaimAmount,-Exposure, -ID)))*predict(xgb_freq, newdata = as.matrix(df[c(6254,24123,25010,25188,30495,30555),]%>% dplyr::select(-ClaimAmount,-ID, -ClaimInd))) #with gender
predict(xgb_gender_sev, newdata = as.matrix(df_gender[c(6254,24123,25010,25188,30495,30555),]%>% dplyr::select(-ClaimAmount,-Exposure)))*predict(xgb_gender_freq, newdata = as.matrix(df_gender[c(6254,24123,25010,25188,30495,30555),]%>% dplyr::select(-ClaimAmount, -ClaimInd))) #without gender
```
        F        F         M           M        F         M
